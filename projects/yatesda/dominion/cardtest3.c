...
int checkMine(int p, struct gameState *initial) {
	...
	j = initial->hand[currentPlayer][choice1];  //store card we will trash


  	gainCard(choice2, initial, 2, currentPlayer);

  	//discard card from hand
  	discardCard(p, currentPlayer, initial, 0);

  	//discard trashed card
  	for (i = 0; i < initial->handCount[currentPlayer]; i++)
	{
	  if (initial->hand[currentPlayer][i] == j)
		{
		  discardCard(i, currentPlayer, initial, 0);
		  break;
		}
	}
